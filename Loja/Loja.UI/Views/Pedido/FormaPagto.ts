        ...
        if (enumFP) {
            for (let i = 0; i < this.lstCoeficiente.length; i++) {
                if (enumFP.toString() == Constantes.COD_FORMA_PAGTO_A_VISTA) {
                    ...
                    this.lstCoeficiente[i].TipoParcela == Constantes.COD_CUSTO_FINANC_FORNEC_TIPO_PARCELAMENTO__SEM_ENTRADA) {
                    this.lstMsg.push(this.lstCoeficiente[i].QtdeParcelas + " X " +
                        this.moedaUtils.formatarMoedaComPrefixo(vlTotalPedido / this.lstCoeficiente[i].QtdeParcelas));
                    ...
                    this.lstCoeficiente[i].TipoParcela == Constantes.COD_CUSTO_FINANC_FORNEC_TIPO_PARCELAMENTO__COM_ENTRADA) {
