...
namespace Pedido.Criacao.Passo60.Gravacao
{
    class Gravacao : PassoBase
    {
        ...
        public async Task Executar()
        {
            ...
            await new Grava15.Grava15(contextoBdGravacao, Pedido, Retorno, Criacao, Execucao).ExecutarAsync();
            if (Retorno.AlgumErro()) return;

            //Passo20: LER AS REGRAS DE CONSUMO DO ESTOQUE
            Execucao.Gravacao.ListaRegrasControleEstoque = await new Grava20.Grava20(contextoBdGravacao, Pedido, Retorno, Criacao, Execucao).ExecutarAsync();

            //Passo25:  VERIFICA SE AS REGRAS ASSOCIADAS AOS PRODUTOS ESTÃO OK - linha 1010
            await new Grava25.Grava25(contextoBdGravacao, Pedido, Retorno, Criacao, Execucao).ExecutarAsync();
            //se tiver erro, nao continua
            if (Retorno.AlgumErro()) return;

            //Passo30: OBTÉM DISPONIBILIDADE DO PRODUTO NO ESTOQUE - linha 1083
            await new Grava30.Grava30(contextoBdGravacao, Pedido, Retorno, Criacao, Execucao).ExecutarAsync();

            //Passo40: Verifica se a disponibilidade do estoque foi alterada - Linha 1159
            await new Grava40.Grava40(contextoBdGravacao, Pedido, Retorno, Criacao, Execucao).ExecutarAsync();

            //Passo50: ANALISA A QUANTIDADE DE PEDIDOS QUE SERÃO CADASTRADOS (AUTO-SPLIT) - linha 1184
            new Grava50.Grava50(contextoBdGravacao, Pedido, Retorno, Criacao, Execucao).Executar();
            ...
            //se tiver erro, nao continua
            if (Retorno.AlgumErro()) return;
            await new Grava60.Grava60(contextoBdGravacao, Pedido, Retorno, Criacao, Execucao).ExecutarAsync();
            ...
            await new Grava80.Grava80(contextoBdGravacao, Pedido, Retorno, Criacao, Execucao).ExecutarAsync();

            //Passo90: log (Passo90/Log.feature)
            await new Grava90.Grava90(contextoBdGravacao, Pedido, Retorno, Criacao, Execucao).ExecutarAsync();
