            ...
            try
            {
                using (var db = contextoProvider.GetContextoLeitura())
                {
                    var orcamento = (from l in db.TorcamentoCotacaoLink
                            ...
                            join u in db.Tusuarios on o.IdVendedor equals u.Id into gg
                    ...
                    if (orcamento != null)
                    {
                        var u3 = db.TorcamentistaEindicadorVendedors.FirstOrDefault(x => x.Id == orcamento.idUsuarioCadastro)?.Nome;
                        var u2 = db.TorcamentistaEindicadors.FirstOrDefault(x => x.IdIndicador == orcamento.idUsuarioCadastro)?.Apelido;
                        var u1 = db.Tusuarios.FirstOrDefault(x => x.Id == orcamento.idUsuarioCadastro)?.Usuario;
